...
export default async function ReceivableLoanDetailPage({
  ...
  if (!detail) notFound();
  const employeeLoan = detail.account.category === "employee_loan"
    ? await getAdminEmployeeLoanDetail(id)
    : null;
    ...
    >
      ...
      {employeeLoan ? (
        <EmployeeLoanAdminPanel
          account={employeeLoan.account}
          extension={employeeLoan.extension}
          permissions={{
            canApprove: isAdmin || permissions.has("receivables.approve"),
            canDisburse: isAdmin || permissions.has("receivables.disburse"),
          }}
