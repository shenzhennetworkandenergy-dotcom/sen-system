...
export default async function RmbPaymentDetailPage({
  ...
  return <DashboardShell admin title={job.job_reference} subtitle="RMB / China Payment Service job detail">
    ...
    <div className="grid gap-4 md:grid-cols-3">
      ...
      <section className="rounded-xl border bg-[var(--surface)] p-4"><h2 className="font-bold">Foreign amount</h2><p className="mt-2 text-xl font-bold">{job.foreign_currency} {money(job.foreign_amount, 4)}</p>{job.extra_service_applied ? <dl className="mt-2 grid grid-cols-[1fr_auto] gap-x-3 text-sm"><dt>Extra service</dt><dd>{money(job.service_percentage || 0, 4)}%</dd><dt>Service amount</dt><dd>{money(job.service_amount || 0, 4)}</dd><dt>Adjusted RMB</dt><dd>{money(job.adjusted_rmb_amount || job.foreign_amount, 4)}</dd></dl> : <p className="mt-2 text-sm text-[var(--muted)]">Extra service: Off</p>}<p className="mt-2 text-sm text-[var(--muted)]">Rate: {money(job.agreed_bdt_rate, 6)}</p></section>
