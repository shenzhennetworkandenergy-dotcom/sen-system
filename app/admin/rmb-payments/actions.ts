...
export type RmbCustomerPreferenceActionState = {
  status: "idle" | "success" | "error";
  message: string;
  customerId: string;
...
function servicePercentage(value: string) {
  if (!/^\d+(?:\.\d{1,4})?$/.test(value)) {
    throw new Error("Service percentage must use up to 4 decimal places.");
  }
  const percentage = Number(value);
  if (!Number.isFinite(percentage) || percentage <= 0 || percentage > 100) {
    throw new Error("Service percentage must be greater than 0 and no more than 100.");
  }
  return value;
}
...
function readValues(form: FormData): RmbPaymentFormValues {
  ...
  return {
    ...
    extra_service_applied: form.get("extra_service_applied") === "on",
    service_percentage: text(form, "service_percentage", 20),
...
export async function saveRmbCustomerPreferenceAction(
  _state: RmbCustomerPreferenceActionState,
  form: FormData,
): Promise<RmbCustomerPreferenceActionState> {
  const { profile } = await requireProfile(["admin"]);
  const customerId = text(form, "customer_id", 100);
  const operation = text(form, "operation", 20);
  try {
    ...
    const { data: customer, error: customerError } = await db
      ...
      .eq("status", "active")
      .maybeSingle();
    if (customerError || !customer) throw new Error("Choose an active customer.");

    if (operation === "remove") {
      const { error } = await db.from("rmb_customer_preferences").delete().eq("customer_id", customerId);
      if (error) throw new Error(error.message);
      revalidatePath("/admin/rmb-payments/new");
      return { status: "success", message: "Customer RMB default removed.", customerId, servicePercentage: null };
    }

    const percentage = servicePercentage(text(form, "service_percentage", 20));
    const { error } = await db.from("rmb_customer_preferences").upsert({
    ...
    return {
      status: "success",
      message: "Customer RMB default saved.",
      customerId,
      servicePercentage: Number(percentage),
    };
  } catch (error) {
    return { status: "error", message: message(error), customerId, servicePercentage: null };
  }
}
...
export async function createRmbPaymentAction(_state: RmbPaymentFormState, form: FormData): Promise<RmbPaymentFormState> {
  ...
  try {
    ...
    const percentage = values.extra_service_applied
      ? servicePercentage(values.service_percentage)
      : null;
    ...
    const { data, error } = await db.rpc("create_rmb_payment_job_v2", {
      ...
      requested_extra_service_applied: values.extra_service_applied,
      requested_service_percentage: percentage,
