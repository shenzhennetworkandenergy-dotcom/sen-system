  ...
  return (
    ...
    >
      ...
      <div className="mb-3 flex flex-wrap gap-2">
        <a href="/admin/crm" className="rounded-lg border bg-[var(--surface)] px-4 py-2 font-bold">CRM overview</a>
        <a href="/admin/crm/chatbot/export" className="rounded-lg border bg-[var(--surface)] px-4 py-2 font-bold">Export inquiries CSV</a>
      </div>
      ...
      <div className="mt-3 flex justify-between text-sm">
        ...
        <div className="flex gap-2">
          {page > 1 ? <a href={`?page=${page - 1}`} className="rounded border px-3 py-1">Previous</a> : null}
          {page * size < (count ?? 0) ? <a href={`?page=${page + 1}`} className="rounded border px-3 py-1">Next</a> : null}
        </div>
      </div>
    </DashboardShell>
  );
