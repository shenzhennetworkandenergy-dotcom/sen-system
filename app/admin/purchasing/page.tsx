...
export default async function PurchasingPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return <DashboardShell admin={profile.role === "admin"} employeePermissions={profile.role === "employee" ? permissions : undefined} title="Purchasing" subtitle="Manage suppliers, purchase orders, approvals, incoming stock and receipts.">
    ...
    <div className="my-3 flex flex-wrap gap-2">{profile.role === "admin" || permissions.has("purchasing.create") ? <a href="/admin/purchasing/new" className="rounded-xl bg-[var(--primary)] px-4 py-2.5 font-bold text-[var(--primary-foreground)]">New purchase order</a> : null}{profile.role === "admin" || permissions.has("suppliers.view") ? <a href="/admin/suppliers" className="rounded-xl border bg-[var(--surface)] px-4 py-2.5 font-bold">Suppliers</a> : null}{profile.role === "admin" || permissions.has("purchasing.export") ? <a href="/admin/purchasing/export" className="rounded-xl border bg-[var(--surface)] px-4 py-2.5 font-bold">Export CSV</a> : null}</div>
