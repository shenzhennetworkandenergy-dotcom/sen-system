...
export default async function ProductsPage({searchParams}:{searchParams:Promise<ProductListParams&{success?:string;error?:string}>}){
  ...
  return <DashboardShell admin={profile.role==="admin"} employeePermissions={profile.role==="employee"?permissions:undefined} title="Products" subtitle="Manage SEN simple and variable products, classification, pricing and stock settings.">
    ...
    <div className="mb-4 flex flex-wrap gap-2">
      {can("products.create")?<a href="/admin/products/new" className="rounded-lg bg-[var(--primary)] px-4 py-2.5 font-semibold text-[var(--primary-foreground)]">Add product</a>:null}
      {can("products.edit")?["Categories","Brands","Attributes"].map((label)=><a key={label} href={`/admin/${label.toLowerCase()}`} className="rounded-lg border px-4 py-2.5 font-semibold">{label}</a>):null}
      {can("inventory.export")?<a href="/admin/inventory/export" className="rounded-lg border px-4 py-2.5 font-semibold">Export inventory CSV</a>:null}
      {can("products.import")?<a href="/admin/products/import" className="rounded-lg border px-4 py-2.5 font-semibold">Import products CSV</a>:null}
    </div>
    ...
    {can("products.archive")?<form id="bulk-archive-form" action={archiveProductAction}/>:null}
    {products.length?<div className="overflow-x-auto rounded-xl border bg-[var(--surface)]"><table className="w-full min-w-[1280px] text-left text-sm">
      ...
      <tbody>{products.map((product)=>{const isPublic=product.status==="active"&&product.public_catalogue_visible;return <tr key={product.id} className="border-b align-top transition hover:bg-cyan-50/40">
        <td className="p-3">{can("products.archive")?<input form="bulk-archive-form" type="checkbox" name="productIds" value={product.id} aria-label={`Select ${product.name}`}/>:null}</td>
        ...
        <td><div className="flex min-w-28 flex-col items-start gap-2">{can("products.edit")?<><ProductQuickEdit product={product}/><a href={`/admin/products/${product.id}`} className="font-semibold text-[var(--primary)]">Full edit</a></>:null}{isPublic?<a href={`/products/${product.slug}`} target="_blank" rel="noreferrer" className="font-semibold text-emerald-700">Preview product ↗</a>:can("products.edit")?<span className="text-xs text-[var(--muted-text)]">Publish to preview</span>:null}</div></td>
      </tr>})}</tbody>
    ...
    {can("products.archive")?<button form="bulk-archive-form" className="mt-4 rounded-lg border px-4 py-2 font-semibold">Archive selected</button>:null}
