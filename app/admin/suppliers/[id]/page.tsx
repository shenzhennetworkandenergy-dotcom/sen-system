...
export default async function SupplierPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{success?:string;error?:string}>}) {
  await connection(); const {profile,permissions}=await requirePermission("suppliers.view"); const {id}=await params; const messages=await searchParams; const data=await getSupplier(id); if(!data) notFound();
