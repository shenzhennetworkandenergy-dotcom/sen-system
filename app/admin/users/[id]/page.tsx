...
export default async function AdminUserDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ success?: string; error?: string }> }) {
  ...
  const deletionMode = await getDeletionMode();
  ...
  const [modules, templates, matrix, authResult, workplaceSummary] = await Promise.all([
    getPermissionCatalogue(),
    getPermissionTemplates(),
    getPermissionMatrix(id),
    supabase.auth.admin.getUserById(id),
    user.role === "employee" ? getEmployeeWorkplaceSummary(id) : Promise.resolve(null),
  ]);
  ...
  const selectedKeys = user.role === "employee" ? matrix.effectiveKeys : [];
  ...
  return <DashboardShell admin title={user.full_name ?? "User detail"} subtitle="Complete profile, account access, employee permissions and activity.">
    ...
    {workplaceSummary ? <div className="mt-6"><EmployeeWorkplaceSummary summary={workplaceSummary} /></div> : null}
    <form action={accountAction} className="mt-6 rounded-2xl border bg-[var(--surface)] p-6"><h2 className="text-xl font-semibold">Role and account status</h2><p className="mt-2 text-sm text-[var(--muted-text)]">An administrator can change any account role. Employee permissions are managed separately and are never shown for customers.</p><div className={`mt-4 grid gap-4 ${user.role === "employee" ? "md:grid-cols-3" : "md:grid-cols-2"}`}><label>Role<select name="role" defaultValue={user.role} className="mt-1 w-full rounded border p-3"><option value="customer">Customer</option><option value="employee">Employee</option><option value="admin">Administrator</option></select></label><label>Status<select name="status" defaultValue={user.status} className="mt-1 w-full rounded border p-3"><option value="active">Active</option><option value="suspended">Suspended</option><option value="disabled">Disabled</option></select></label>{user.role === "employee" ? <label>Employee baseline<select name="templateId" defaultValue={selectedTemplate?.id} className="mt-1 w-full rounded border p-3" disabled={!templates.length}>{templates.map((template) => <option key={template.id} value={template.id}>{template.name}{template.is_default ? " (Default)" : ""}</option>)}</select></label> : <input type="hidden" name="templateId" value=""/>}</div><p className="mt-3 text-sm text-[var(--muted-text)]">The final active administrator cannot be downgraded or suspended.</p>{user.role === "employee" && selectedTemplate ? <div className="mt-6"><h3 className="mb-4 text-lg font-semibold">Employee baseline preview</h3><PermissionChecklist modules={modules} initialSelected={selectedKeys} templateKeys={selectedTemplate.permissionKeys} allowKeys={matrix.allowKeys} denyKeys={matrix.denyKeys}/></div> : user.role === "admin" ? <p className="mt-6 rounded border bg-[var(--muted-surface)] p-4 font-semibold">Administrators have unrestricted access.</p> : <p className="mt-6 rounded border bg-[var(--muted-surface)] p-4 text-sm">Customer accounts have no staff-permission controls. Choose Employee and save to assign the default staff template.</p>}<button className="mt-5 rounded bg-[var(--primary)] px-4 py-3 font-semibold text-[var(--primary-foreground)]">Save account access</button></form>
    {user.role === "employee" ? <section className="mt-6 rounded-2xl border bg-[var(--surface)] p-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-semibold">Specific employee permissions</h2><p className="mt-1 text-sm text-[var(--muted-text)]">These settings apply only to this employee.</p></div><a href={`/admin/permissions?employee=${id}`} className="rounded border px-4 py-2 text-sm font-semibold">Open focused permission editor</a></div>{selectedTemplate ? <form action={permissionAction} className="mt-5"><label className="mb-5 block">Baseline template<select name="templateId" defaultValue={selectedTemplate.id} className="mt-1 w-full rounded border p-3 md:max-w-md">{templates.map((template) => <option key={template.id} value={template.id}>{template.name}{template.is_default ? " (Default)" : ""}</option>)}</select></label><p className="mb-4 text-sm"><b>Effective:</b> {matrix.effectiveKeys.length} · Template: {matrix.templateKeys.length} · Additional grants: {matrix.allowKeys.length} · Removed: {matrix.denyKeys.length}</p><PermissionChecklist modules={modules} initialSelected={matrix.effectiveKeys} templateKeys={selectedTemplate.permissionKeys} allowKeys={matrix.allowKeys} denyKeys={matrix.denyKeys}/><button className="mt-5 rounded bg-[var(--primary)] px-4 py-3 font-semibold text-[var(--primary-foreground)]">Save this employee’s permissions</button></form> : <p className="mt-3 text-red-800">No active employee template is available.</p>}</section> : null}
    ...
    <section className="mt-6 rounded-2xl border border-red-300 bg-red-50 p-6 text-red-950"><h2 className="text-xl font-semibold">Remove user account</h2><p className="mt-2 text-sm">{deletionMode.permanentEnabled ? "Permanent Deletion Mode is enabled. An unused account will be removed permanently; accounts owning protected business records cannot be purged." : "Archive protection is enabled. The account will be disabled, hidden from the main user list, and moved to the recoverable Archive."}</p><form action={deleteUserAction.bind(null, id)}><ConfirmSubmitButton confirmation={`${deletionActionCopy(deletionMode.permanentEnabled).confirmation} User: ${user.full_name ?? user.email ?? "this user"}`} className="mt-4 rounded border border-red-700 bg-white px-4 py-3 font-semibold text-red-800">{deletionActionCopy(deletionMode.permanentEnabled).button}</ConfirmSubmitButton></form></section>
  </DashboardShell>;
}
