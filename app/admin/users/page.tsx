...
const allowedRoles = new Set<string>(["customer", "employee", "admin"]);
const allowedStatuses = new Set<string>(["active", "suspended", "disabled"]);
...
export default async function AdminUsersPage({ searchParams }: { searchParams: Promise<{ q?: string; role?: string; status?: string }> }){
  ...
  let query=supabase.from("profiles").select("id,email,full_name,role,status,country_code,country_name,customer_type,updated_at").order("updated_at",{ascending:false}).limit(100);
