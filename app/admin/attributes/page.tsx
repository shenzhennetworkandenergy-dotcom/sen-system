...
import { createAttributeAction, createAttributeValueAction, deleteAttributeAction } from "../catalog-actions";
...
export default async function AttributesPage({ searchParams }: { searchParams: Promise<{ success?: string; error?: string }> }) {
  ...
  const [{ data: attributes }, { data: values }, message, mode] = await Promise.all([db.from("attributes").select("id,name,slug,description,is_active").eq("is_active",true).order("sort_order"), db.from("attribute_values").select("id,attribute_id,value,slug").order("sort_order"), searchParams, getDeletionMode()]);
