...
export default async function SerialsPage({ searchParams }: { searchParams: Promise<{ q?: string; product?: string; status?: string; warehouse?: string; page?: string }> }) {
  ...
  return <DashboardShell admin={profile.role === "admin"} employeePermissions={profile.role === "employee" ? permissions : undefined} title="Serial Tracking" subtitle="Trace serialized SEN stock by product, warehouse, status, and movement history.">
    ...
    <form className="mb-5 grid gap-3 rounded-xl border bg-[var(--surface)] p-4 sm:grid-cols-2 lg:grid-cols-5"><input name="q" aria-label="Search serials" placeholder="Serial or barcode" defaultValue={params.q} className="rounded border p-2" /><select name="product" defaultValue={params.product} className="rounded border p-2"><option value="">All products</option>{(productOptions ?? []).map((product) => <option key={product.id} value={product.id}>{product.name} ({product.sku})</option>)}</select><select name="status" defaultValue={params.status} className="rounded border p-2"><option value="">All statuses</option>{["available", "reserved", "sold", "damaged", "unavailable", "transferred", "removed"].map((status) => <option key={status}>{status}</option>)}</select><select name="warehouse" defaultValue={params.warehouse} className="rounded border p-2"><option value="">All warehouses</option>{(warehouses ?? []).map((warehouse) => <option key={warehouse.id} value={warehouse.id}>{warehouse.name} ({warehouse.code})</option>)}</select><button className="rounded border px-4 py-2 font-semibold">Apply filters</button></form>
