...
export default async function SerialBatchesPage({ searchParams }: { searchParams: Promise<{ success?: string; error?: string; product?: string }> }) {
  const { profile, permissions } = await requireAnyPermission(["serials.view", "inventory.receive"]);
  ...
  return <DashboardShell admin={profile.role === "admin"} employeePermissions={profile.role === "employee" ? permissions : undefined} title="Serial generation batches" subtitle="Generated labels are expected units until you receive their batch into stock.">
    ...
    {selectedProduct ? <div className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-blue-200 bg-blue-50 p-4 text-blue-950"><div><span className="text-xs font-bold uppercase tracking-wider">Receiving stock for</span><strong className="block">{selectedProduct.name}</strong><span className="text-sm">{selectedProduct.sku}</span></div><div className="flex gap-2">{profile.role === "admin" || permissions.has("serials.view") ? <a href={`/admin/serials?product=${selectedProduct.id}`} className="rounded border border-blue-300 bg-white px-3 py-2 font-semibold">View serials</a> : null}{profile.role === "admin" || permissions.has("products.view") ? <a href={`/admin/products/${selectedProduct.id}`} className="rounded border border-blue-300 bg-white px-3 py-2 font-semibold">Product details</a> : null}</div></div> : null}
      ...
      <table className="w-full min-w-[860px] text-left">
        ...
        <tbody>{batches.map((batch) => <tr key={batch.id} className="border-t">
          ...
          <td><div className="flex flex-wrap items-center gap-2">
            {(profile.role === "admin" || permissions.has("inventory.receive")) && (batch.status === "generated" || batch.status === "partially_received") ? <form action={receiveExpectedSerialBatchAction}><input type="hidden" name="batch_id" value={batch.id}/><input type="hidden" name="return_product" value={batch.product_id}/><button className="rounded bg-[var(--primary)] px-3 py-2 text-sm font-semibold text-white">Receive into stock</button></form> : null}
            {profile.role === "admin" || permissions.has("serials.print") ? <a href={`/admin/serials/print?batch=${batch.id}`} className="font-semibold">Print</a> : null}
            {profile.role === "admin" || permissions.has("serials.export") ? <a href={`/admin/serials/export?batch=${batch.id}`}>CSV</a> : null}
          </div></td>
        </tr>)}</tbody>
      </table>
