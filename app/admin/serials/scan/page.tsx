...
export default async function ScanPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return <DashboardShell admin={profile.role === "admin"} employeePermissions={profile.role === "employee" ? permissions : undefined} title="Scan serial" subtitle="Use manual input, debounced autocomplete, a keyboard scanner, or an explicitly activated camera.">
    <form className="mb-4 flex items-start gap-2"><SerialSearchField initialValue={q} canPrint={canPrint}/><button className="min-h-12 rounded bg-[var(--primary)] px-5 font-semibold text-[var(--primary-foreground)]">Search</button></form>
    <SerialScanner />
    {q ? <section className="mt-5 rounded-xl border bg-[var(--surface)] p-5"><h2 className="font-semibold">Results</h2>{results.length ? <ul className="mt-3 divide-y">{results.map((result) => <li key={String(result.id)} className="flex items-center justify-between gap-3 py-3"><div><a className="font-mono text-sm font-semibold" href={`/admin/serials/${result.id}`}>{String(result.sen_serial)}</a><span className="block text-sm">{String(result.manufacturer_serial ?? "Not provided")} · {String(result.status)} · {String(result.condition)}</span></div>{canPrint ? <SerialPrintLink serialId={String(result.id)}>Print</SerialPrintLink> : null}</li>)}</ul> : <p className="mt-3">No matching serials.</p>}</section> : null}
  </DashboardShell>;
}
