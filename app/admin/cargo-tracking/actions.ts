...
export async function updateCargoTrackingNumberAction(jobId: string, form: FormData) {
  await requirePermission("cargo.create");
  const path = `/admin/cargo-tracking/${jobId}/edit-tracking`;
  const trackingNumber = value(form, "courier_tracking_number").slice(0, 160);
  try {
    if (!trackingNumber) throw new Error("Courier tracking number is required.");
    const db = createSupabaseAdminClient();
    const { data, error } = await db.from("cargo_shipping_jobs")
      .update({ courier_tracking_number: trackingNumber })
      .eq("id", jobId)
      .in("current_status", cargoTrackingEditableStatuses)
      .select("id")
      .maybeSingle();
    if (error?.code === "23505") throw new Error("This courier tracking number already has a cargo job.");
    if (error) throw new Error("Unable to update the tracking number.");
    if (!data) {
      const { data: job } = await db.from("cargo_shipping_jobs").select("id,current_status").eq("id", jobId).maybeSingle();
      if (job) throw new Error("Tracking number can no longer be edited because this cargo has already been dispatched from China.");
      throw new Error("Cargo job not found.");
    }
  } catch (error) {
    go(path, "error", error instanceof Error ? error.message : "Unable to update the tracking number.");
  }
  revalidatePath(`/admin/cargo-tracking/${jobId}`);
  go("/admin/cargo-tracking", "success", "Tracking number updated.");
}
