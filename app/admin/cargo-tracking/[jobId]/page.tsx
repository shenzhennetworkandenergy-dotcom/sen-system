...
import { cargoLabel, cargoPermissionForStatus, getCargoJob, nextCargoStatus, type CargoPermissionKey, type CargoStatus } from "@/lib/cargo-tracking/data";
import {
...
export default async function CargoJobPage({ params, searchParams }: { params: Promise<{ jobId: string }>; searchParams: Promise<{ success?: string; error?: string }> }) {
  ...
  const can = (permission: CargoPermissionKey) => admin || permissions.has(permission);
  ...
  const canAdvance = nextStatus ? can(cargoPermissionForStatus(nextStatus)) : false;
  ...
  return <DashboardShell admin={admin} employeePermissions={admin ? undefined : permissions} title={job.internal_cargo_id} subtitle={`Courier tracking: ${job.courier_tracking_number}`}>
      ...
      <div className="space-y-5">
        ...
        <section className="rounded-xl border bg-[var(--surface)]"><h2 className="p-4 text-lg font-bold">Packages ({packages.length})</h2><div className="space-y-4 border-t p-4">{packages.map((item) => {
          ...
          return <article key={item.id} className="rounded-xl border p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-black text-blue-900">{item.package_identifier}</p><p className="font-semibold">{item.description}</p><p className="text-sm text-[var(--muted-text)]">{item.quantity} {item.unit} · Weight {item.weight ?? "—"} kg · CBM {item.cbm ?? "—"}{item.dimensions ? ` · ${item.dimensions}` : ""}</p></div><Link href={`/admin/cargo-tracking/${jobId}/packages/${item.id}/label`} className="rounded-lg border px-3 py-2 text-sm font-bold">Print Label</Link></div><div className="mt-3 grid gap-2 text-sm sm:grid-cols-3"><p className={item.china_received_at ? "text-green-700" : "text-amber-700"}><strong>China:</strong> {item.china_received_at ? `Received ${new Date(item.china_received_at).toLocaleString("en-GB")}` : "Not verified"}</p><p><strong>Location:</strong> {item.warehouseLocation ? `${item.warehouseLocation.code} ${item.warehouseLocation.name}` : "Not assigned"}</p><p className={item.handed_over_at ? "text-green-700" : item.ready_verified_at ? "text-blue-700" : "text-amber-700"}><strong>Customer:</strong> {item.handed_over_at ? "Handed over" : item.ready_verified_at ? "Ready verified" : "Not ready"}</p></div>
            <div className="mt-3 flex flex-wrap gap-2">{can("cargo.china_receive") && !item.china_received_at && ["expected_china", "received_china"].includes(job.current_status) ? <form action={chinaAction}><button className="rounded-lg border px-3 py-2 text-sm font-bold">China Receive</button></form> : null}{can("cargo.assign_location") && !item.handed_over_at && packageLocationStatuses.includes(job.current_status) && job.bangladesh_warehouse_id ? <form action={locationAction} className="flex flex-wrap gap-2"><select name="location_id" required defaultValue={item.warehouse_location_id ?? ""} className="rounded-lg border bg-white px-3 py-2 text-sm"><option value="">Assign BD location</option>{packageLocations.map((location) => <option key={location.id} value={location.id}>{location.code} · {location.name}</option>)}</select><button className="rounded-lg border px-3 py-2 text-sm font-bold">Save Location</button></form> : null}{can("cargo.mark_ready") && !item.ready_verified_at && item.china_received_at && item.warehouse_location_id && packageReadyStatuses.includes(job.current_status) ? <form action={readyAction}><button className="rounded-lg border border-green-600 px-3 py-2 text-sm font-bold text-green-800">Mark Ready</button></form> : null}</div>
          </article>;
        })}</div></section>
