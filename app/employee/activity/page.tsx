...
export default async function EmployeeActivityPage() {
  await connection();
  const { profile, permissions } = await requirePermission("activity.view_own");
  ...
  return <DashboardShell employeePermissions={permissions} title="My Activity" subtitle="Your recent safe SEN account activity.">{error?<p className="rounded border border-red-200 bg-red-50 p-4 text-red-900">Unable to load your activity.</p>:<ActivityTable rows={(data??[]) as ActivityRow[]} people={people}/>}</DashboardShell>;
}
