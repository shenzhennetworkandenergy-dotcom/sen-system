  ...
  return <section className="space-y-4 rounded-xl border bg-slate-50 p-5">
    ...
    {Array.from({ length: count }, (_, index) => <div key={index} className="rounded-xl border bg-white p-4">
      <div className="mb-3 flex items-center justify-between gap-3">
        ...
        {index >= 3 && index === count - 1 ? <button type="button" onClick={() => setCount((value) => Math.max(3, value - 1))} className="text-sm font-semibold text-red-700">Remove</button> : null}
      </div>
    ...
    <button type="button" disabled={count >= 20} onClick={() => setCount((value) => Math.min(20, value + 1))} className="rounded-lg border border-blue-800 px-4 py-2 font-semibold text-blue-800 disabled:opacity-40">+ Add Witness</button>
  </section>;
}
