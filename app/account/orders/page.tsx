...
export default async function CustomerOrdersPage({
  ...
  const { orders, count, page, size } = await getOrders(params, profile.id);

  return (
    <DashboardShell
      title="My Orders"
      subtitle="Track every order through five clear delivery stages."
    >
          ...
          >
            ...
            {[
              ...
              <option key={status} value={status}>
                {status === "draft" ? "Awaiting confirmation" : label(status)}
              </option>
            ))}
          </select>
          <button className="ml-2 rounded-xl border px-4 py-2 font-semibold">
            Filter
          </button>
        </form>
      </div>
      <div className="grid gap-5 md:grid-cols-2">
        {orders.map((order) => {
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between gap-3 border-b bg-[var(--muted-surface)] px-5 py-4">
                <div>
              ...
              <div className="p-5">
                <p className="text-sm text-[var(--muted-text)]">Order total</p>
                ...
                <a
                  ...
                  className="mt-5 block rounded-xl bg-[var(--primary)] px-4 py-3 text-center font-semibold text-[var(--primary-foreground)]"
                >
      ...
      {!orders.length ? (
        <p className="rounded-2xl border bg-[var(--surface)] p-12 text-center text-[var(--muted-text)]">
