...
export async function saveAddressAction(form: FormData) {
  ...
  try {
    ...
    redirect(target("success", id ? "Address updated." : "Address added."));
  } catch (error) {
    // Next.js uses a thrown internal signal for redirect(). Let it continue rather
    // than turning a successful save into the generic error message.
    if (isRedirectError(error)) throw error;
    console.error("Address save failed", { message: error instanceof Error ? error.message : "Unknown" });
    redirect(target("error", error instanceof Error && /required|invalid|default/i.test(error.message) ? error.message : "Unable to save address."));
  }
}
...
export async function deleteAddressAction(form: FormData) {
  ...
  revalidatePath("/account/addresses"); revalidatePath("/account"); redirect(target("success", "Address deleted."));
}
