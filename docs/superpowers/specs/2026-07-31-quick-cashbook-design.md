...
- Show three summary cards for the selected date: total income, total expense, and net balance.
...
`cashbook_descriptions` stores reusable Income/Expense descriptions. `cashbook_entries` stores the business transaction, payment method, transaction timestamp, Bangladesh business date, creator, and linked journal entry.
...
- Unit tests cover input normalization, Bangladesh business-date selection, daily totals, and invalid values.
