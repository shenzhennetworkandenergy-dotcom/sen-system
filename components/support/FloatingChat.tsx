...
export function FloatingChat({ authenticated, conversation }: {
  ...
  return <div className="sen-floating-chat">
    {open ? <section className="sen-messenger-window" aria-label="Chat with SEN" aria-live="polite">
      <header className="sen-messenger-header">
        <div className="sen-messenger-avatar" aria-hidden="true">S</div>
        <div className="min-w-0 flex-1"><strong className="block truncate">SEN Customer Care</strong><span className="flex items-center gap-1 text-xs text-slate-500"><i className="h-2 w-2 rounded-full bg-emerald-500"/> We usually reply shortly</span></div>
        <button type="button" onClick={() => setOpen(false)} aria-label="Close chat" className="sen-messenger-icon-button">×</button>
      </header>
      {!authenticated ? <div className="p-5 text-sm text-slate-700">
        <p>Sign in to message SEN and keep your conversation history.</p>
        <div className="mt-4 flex gap-2"><Link href="/login" className="rounded-full bg-blue-600 px-4 py-2 font-bold text-white">Login</Link><Link href="/register" className="rounded-full border px-4 py-2 font-bold">Create account</Link></div>
        <Link href="/forgot-password" className="mt-4 block font-semibold text-blue-700">Forgot password? Chat with an administrator</Link>
      </div> : conversation ? <>
        <div className="sen-messenger-messages">
          <div className="mb-4 text-center"><div className="sen-messenger-avatar mx-auto" aria-hidden="true">S</div><strong className="mt-2 block">SEN Customer Care</strong><span className="text-xs text-slate-500">{conversation.subject}</span></div>
          {conversation.messages.map(message => <div key={message.id} className={`sen-messenger-row ${message.is_customer ? "is-customer" : "is-sen"}`}>
            <div className="sen-messenger-bubble"><p>{message.body}</p>{message.attachments.map(attachment => attachment.mime_type.startsWith("image/") ?
              ...
              <a key={attachment.id} href={`/support/attachments/${attachment.id}`} className="mt-2 block font-semibold underline">{attachment.original_file_name}</a>)}</div>
        ...
        <form action={sendFloatingMessageAction.bind(null, conversation.id)} className="sen-messenger-composer">
          ...
          <textarea name="body" rows={1} placeholder="Aa" aria-label="Message"/>
        ...
        <input type="hidden" name="return_path" value={pathname}/>
        <div className="rounded-xl bg-blue-50 p-3 text-sm text-slate-700">Hello! Tell us how SEN can help you today.</div>
    ...
    <button type="button" onClick={() => setOpen(value => !value)} aria-expanded={open} className="sen-chat-trigger"><span aria-hidden="true">💬</span><span>{open ? "Close" : "Chat"}</span></button>
  </div>;
}
