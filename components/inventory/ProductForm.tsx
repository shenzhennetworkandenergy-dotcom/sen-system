...
export function ProductForm({ action, options, product }: { action: (formData: FormData) => void | Promise<void>; options: Options; product?: Product }) {
  ...
  return <form action={action} onInvalid={handleInvalid} onSubmit={validateProduct} className="space-y-6">
    ...
    <section className="rounded-xl border bg-[var(--surface)] p-6"><h2 className="text-xl font-semibold">General</h2><div className="mt-4 grid gap-4 md:grid-cols-2">
      <label>Name{required}<input name="name" required autoFocus={!product} defaultValue={value("name")} className={input}/><span className="mt-1 block text-xs text-[var(--muted-text)]">The public product URL is generated automatically from this title every time you save.</span></label>
    ...
    <section className="rounded-xl border bg-[var(--surface)] p-6">
      <h2 className="text-xl font-semibold">Shipping, warranty and advanced</h2>
      <div className="mt-4 grid gap-4 md:grid-cols-4">{["weight", "length", "width", "height"].map((key) => <label key={key}>{key[0].toUpperCase() + key.slice(1)}<input type="number" min="0" step="0.0001" name={key} defaultValue={value(key)} className={input}/></label>)}</div>
      <div className="mt-4 grid gap-4 md:grid-cols-2"><label>Country of origin<input name="country_of_origin" defaultValue={value("country_of_origin")} className={input}/></label><label>Legacy warranty information<input name="warranty_information" defaultValue={value("warranty_information")} className={input}/></label></div>
      <div className="mt-5 rounded-xl border border-emerald-200 bg-emerald-50/60 p-4">
        <label className="flex items-center gap-2 font-semibold text-emerald-950"><input type="checkbox" name="warranty_enabled" defaultChecked={Boolean(product?.warranty_enabled)}/> Include customer warranty with this product</label>
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          <label>Warranty duration (months)<input type="number" min="0" max="240" step="1" name="warranty_duration_months" defaultValue={value("warranty_duration_months", "12")} className={input}/></label>
          <span className="self-end rounded-lg bg-white p-3 text-sm text-[var(--muted-text)]">Coverage starts when the customer order is delivered.</span>
          <label>Warranty terms<textarea name="warranty_terms" defaultValue={value("warranty_terms")} className="mt-1 min-h-28 w-full rounded-lg border border-slate-300 bg-white p-3"/></label>
          <label>Warranty exclusions<textarea name="warranty_exclusions" defaultValue={value("warranty_exclusions")} className="mt-1 min-h-28 w-full rounded-lg border border-slate-300 bg-white p-3"/></label>
        </div>
      </div>
      <details className="mt-4 rounded-lg border bg-slate-50 p-3"><summary className="cursor-pointer font-semibold">Legacy and extra specifications JSON</summary><label className="mt-3 block">Specifications JSON<textarea name="specifications" defaultValue={value("specifications", "{}")} onInput={(event) => event.currentTarget.setCustomValidity("")} className="mt-1 min-h-24 w-full rounded border bg-white p-3 font-mono text-sm"/><span className="mt-1 block text-xs text-[var(--muted-text)]">Category fields above are authoritative. Unrelated legacy keys in this JSON object are preserved.</span></label></details>
      <label className="mt-4 block">Internal notes<textarea name="internal_notes" defaultValue={value("internal_notes")} className="mt-1 min-h-24 w-full rounded border p-3"/></label>
      <div className="mt-4 flex flex-wrap gap-4"><label><input type="checkbox" name="featured" defaultChecked={Boolean(product?.featured)}/> Featured</label><label className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 font-semibold text-blue-950"><input type="checkbox" name="public_catalogue_visible" defaultChecked={product ? Boolean(product.public_catalogue_visible) : true}/> Show on public Products page</label></div>
    </section>
