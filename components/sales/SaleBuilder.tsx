...
import { createSaleFieldVisibility } from "@/components/sales/sale-builder-visibility";
...
export function SaleBuilder({
  customers, products, variations, balances, warehouses, addresses, role,
}: {
  customers:Customer[]; products:Product[]; variations:Variation[]; balances:Balance[]; warehouses:Warehouse[]; addresses:Address[];
  role:string;
}) {
  const visibility = createSaleFieldVisibility(role);
  const productRowClass = visibility.variation || visibility.lineDiscounts
    ? "grid gap-2 rounded-xl border p-3 lg:grid-cols-[2fr_1fr_.6fr_.8fr_.7fr_.7fr_auto]"
    : "grid gap-2 rounded-xl border p-3 lg:grid-cols-[2fr_.6fr_.8fr_auto]";
  ...
  return <form action={createSaleAction} className="space-y-4">
    ...
    <section className="grid gap-4 rounded-xl border bg-[var(--surface)] p-4 lg:grid-cols-3">
        ...
        onSelectionChange={(selectedCustomer) => {
          ...
          setAddressId(
            selectedCustomer
              ? addresses.find(
                  (address) =>
                    address.profile_id === selectedCustomer.id &&
      ...
      {visibility.deliveryDetails && !addressId ? <>
    ...
    <section className="rounded-xl border bg-[var(--surface)] p-4">
      ...
      <div className="mt-3 space-y-3">{selected.map((row)=><article key={row.key} className={productRowClass}>
        <SaleProductPicker
          products={products}
        ...
        />
        {visibility.variation ? <label className="text-xs font-semibold">Variation<select value={row.variation_id} onChange={(event)=>{const variation=variations.find((item)=>item.id===event.target.value),price=Math.round(Number(variation?.sale_price??variation?.regular_price??row.catalogue_price)*100)/100;update(row.key,{variation_id:event.target.value,unit_price:String(price),catalogue_price:price})}} className={field}><option value="">None</option>{variations.filter((item)=>item.product_id===row.product_id).map((item)=><option key={item.id} value={item.id}>{item.name||item.sku}</option>)}</select></label> : null}
    ...
    <section className="grid gap-3 rounded-xl border bg-[var(--surface)] p-4 md:grid-cols-3">{visibility.orderAdjustments ? <><label className="text-sm">Order discount<input name="discount_amount" type="number" min="0" step=".01" defaultValue="0" className={field}/></label><label className="text-sm">Shipping charge<input name="shipping_amount" type="number" min="0" step=".01" defaultValue="0" className={field}/></label><label className="text-sm">Installation/service<input name="service_amount" type="number" min="0" step=".01" defaultValue="0" className={field}/></label><label className="text-sm">VAT/tax<input name="tax_amount" type="number" min="0" step=".01" defaultValue="0" className={field}/></label><label className="text-sm md:col-span-2">Discount reason<input name="discount_reason" className={field}/></label><label className="text-sm md:col-span-3">Customer note<textarea name="customer_notes" className={field}/></label><label className="text-sm md:col-span-3">Internal note<textarea name="internal_notes" className={field}/></label></> : null}<div className="md:col-span-3 text-right text-xl font-bold">Items after line discounts: {subtotal.toFixed(2)} BDT</div></section>
