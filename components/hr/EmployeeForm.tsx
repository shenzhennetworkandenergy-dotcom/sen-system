...
type Reference = { id: string; name?: string | null; full_name?: string | null; email?: string | null; role?: string; status?: string };
...
export function EmployeeForm({
  ...
  refs: { departments: Reference[]; teams: Reference[]; designations: Reference[]; profiles: Reference[]; locations: Reference[] };
  ...
  return (
    <form action={saveEmployeeAction} className="space-y-5">
      ...
      <section className="rounded-2xl border bg-[var(--surface)] p-5">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          <label className="text-sm font-semibold">Base salary (BDT)<input type="number" min="0" step="0.01" name="base_salary" defaultValue={value(record,"base_salary")} className={hrField}/></label>
          <label className="text-sm font-semibold">Currency<input name="salary_currency" defaultValue={value(record,"salary_currency") || "BDT"} maxLength={3} className={hrField}/></label>
