      ...
      <table className="w-full min-w-[900px] text-left text-sm">
        ...
        <tbody>{openItems.map((item) => {
          ...
          return <tr key={item.id} className="border-t align-top">
            ...
            <td className="p-3"><input aria-label={`Receive ${item.product_name_snapshot}`} type="number" min="0" max={item.remaining} step={item.products?.serial_tracking_required ? "1" : "0.0001"} value={value?.quantity ?? 0} onChange={(event) => setReceived((current) => ({ ...current, [item.id]: { ...current[item.id], quantity: Number(event.target.value) } }))} className="w-28 rounded-lg border px-2 py-1.5" /></td>
