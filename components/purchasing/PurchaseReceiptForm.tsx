      ...
      <table className="w-full min-w-[900px] text-left text-sm">
        ...
        <tbody>{openItems.map((item) => {
          ...
          return <tr key={item.id} className="border-t align-top">
            ...
            <td className="p-3">{item.products?.serial_tracking_required ? <div className="grid gap-2 sm:grid-cols-[130px_1fr]"><select aria-label={`Condition for ${item.product_name_snapshot}`} value={value?.condition ?? "new"} onChange={(event) => setReceived((current) => ({ ...current, [item.id]: { ...current[item.id], condition: event.target.value } }))} className="rounded-lg border px-2 py-1.5"><option value="new">New</option><option value="refurbished">Refurbished</option><option value="used">Used</option></select><div><textarea aria-label={`Manufacturer serials for ${item.product_name_snapshot}`} value={value?.serials ?? ""} onChange={(event) => setReceived((current) => ({ ...current, [item.id]: { ...current[item.id], serials: event.target.value } }))} placeholder="Optional manufacturer serials, one per line" className="min-h-20 w-full rounded-lg border px-2 py-1.5" /><span className="mt-1 block text-xs font-semibold text-emerald-700">The confirmed SEN serials activate automatically when this receipt is saved.</span></div></div> : <span className="text-[var(--muted-text)]">Non-serialized stock</span>}</td>
          </tr>;
        })}</tbody>
      </table>
    </div>
    {!openItems.length ? <p className="rounded-xl border bg-[var(--muted-surface)] p-5 text-center">All order quantities have been received.</p> : <button disabled={!payload.length} className="rounded-xl bg-[var(--primary)] px-6 py-3 font-bold text-[var(--primary-foreground)] disabled:opacity-40">Confirm stock receipt and generate SEN serials</button>}
  </form>;
}
