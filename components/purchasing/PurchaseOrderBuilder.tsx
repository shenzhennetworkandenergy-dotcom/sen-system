  ...
  return <form action={action} className="space-y-4">
    <section className="grid gap-3 rounded-2xl border bg-[var(--surface)] p-4 shadow-sm md:grid-cols-2 xl:grid-cols-4">
      ...
      <label className="text-sm font-semibold">Currency *
        <CurrencyCombobox key={`${supplierId}:currency`} name="currency" required defaultValue={defaults.currency ?? supplier?.default_currency ?? "BDT"} className="mt-1 w-full rounded-xl border px-3 py-2.5 uppercase" />
      </label>
      ...
      <div className="mt-4 overflow-x-auto">
        <table className="w-full min-w-[1050px] text-left text-sm">
          ...
          <tbody>{items.map((item, index) => <tr key={`${item.product_id}:${item.variation_id ?? ""}`} className="border-t align-top">
            <td className="p-3"><strong>{item.name}</strong>{item.serial_tracking_required ? <span className="mt-1 block text-xs text-cyan-700">Serialized units · Unique SEN serials generate automatically at stock receipt</span> : null}<input aria-label={`Description for ${item.name}`} value={item.description} onChange={(event) => updateItem(index, { description: event.target.value })} placeholder="Optional line note" className="mt-2 w-full rounded-lg border px-2 py-1.5" /></td>
