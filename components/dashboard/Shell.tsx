...
export function DashboardShell({ title, subtitle, children, admin = false, employeePermissions }: { title: string; subtitle: string; children: React.ReactNode; admin?: boolean; employeePermissions?: Iterable<string> }) {
  ...
  return <div className="min-h-screen bg-[var(--muted-surface)]">
    ...
    <div className={`mx-auto grid max-w-7xl gap-6 px-6 py-8 ${navigation.length ? "lg:grid-cols-[16rem_1fr]" : ""}`}>
      {navigation.length ? <aside className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-4"><nav className="space-y-1 text-sm">{navigation.map((item) => <a key={item.key} href={item.route!} className="block rounded-lg px-3 py-2 font-medium hover:bg-[var(--muted-surface)]">{item.label}</a>)}</nav></aside> : null}
      <main><div className="mb-6"><h1 className="text-3xl font-bold">{title}</h1><p className="mt-2 text-[var(--muted-text)]">{subtitle}</p></div>{children}</main>
    </div>
  </div>;
}
