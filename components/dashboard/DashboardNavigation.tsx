...
import { dashboardToneForGroup, dashboardToneForModule, type DashboardNavigationItem } from "@/lib/navigation/dashboard";
...
export function DashboardNavigation({items,workCounts={}}:{items:DashboardNavigationItem[];workCounts?:Record<string,number>}){
  ...
  return <>
    ...
    <aside id="dashboard-navigation" className={`${open?"translate-x-0":"-translate-x-full"} sen-dashboard-sidebar fixed inset-y-0 left-0 z-50 w-[min(18rem,88vw)] overflow-y-auto border-r border-slate-700 bg-[#0b1730] p-3 text-slate-100 shadow-2xl transition-transform lg:sticky lg:top-[4.25rem] lg:z-auto lg:h-[calc(100vh-5.25rem)] lg:w-auto lg:translate-x-0 lg:rounded-xl lg:border`}>
      ...
      <nav aria-label="Dashboard navigation" className="space-y-3">{groups.map((group)=><section key={group} data-dashboard-tone={dashboardToneForGroup(group)}><h2 className="mb-1 px-2 text-[10px] font-bold uppercase tracking-[0.12em] text-cyan-200/70">{group}</h2><div className="space-y-0.5">{items.filter((item)=>item.group===group).map((item)=>{const routeHasQuery=Boolean(item.route?.includes("?"));const active=Boolean(item.route&&matchesRoute(item.route)&&(!specificMatch||routeHasQuery));const count=workCounts[item.key]??0;const tone=dashboardToneForModule(item);return item.route&&item.implemented?<a key={item.key} href={item.route} aria-current={active?"page":undefined} data-dashboard-tone={dashboardToneForModule(item)} className={`group flex min-h-8 items-center gap-2 rounded-lg px-2 py-1.5 text-[13px] font-medium transition-all ${active?"bg-gradient-to-r from-cyan-500 to-blue-600 text-white shadow-md shadow-blue-950/30":count>0?"bg-amber-400/15 text-amber-100 ring-1 ring-amber-300/40 hover:bg-amber-400/25":"text-slate-200 hover:bg-white/10 hover:text-white"}`}><span className="sen-dashboard-module-mark" data-dashboard-tone={tone}><NavigationIcon name={item.iconKey}/></span><span className="min-w-0 flex-1 truncate">{item.label}</span>{count>0?<span className="grid min-w-5 place-items-center rounded-full bg-red-500 px-1.5 py-0.5 text-[10px] font-bold text-white shadow" aria-label={`${count} items need attention`}>{count}</span>:null}</a>:<span key={item.key} aria-disabled="true" data-dashboard-tone={dashboardToneForModule(item)} className="flex min-h-8 items-center gap-2 rounded-lg px-2 py-1.5 text-[13px] text-slate-400"><span className="sen-dashboard-module-mark" data-dashboard-tone={tone}><NavigationIcon name={item.iconKey}/></span><span className="min-w-0 flex-1 truncate">{item.label}</span><span className="rounded bg-white/10 px-1.5 py-0.5 text-[8px] font-bold uppercase tracking-wide text-slate-300">Planned</span></span>})}</div></section>)}</nav>
    </aside>
  </>;
}
