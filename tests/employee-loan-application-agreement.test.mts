...
test("employee application and Admin terms validate independently", () => {
  ...
  assert.equal(normalizeEmployeeLoanApplication({ ...request, witnesses: request.witnesses.slice(0, 2) }).witnesses.length, 2);
  assert.equal(normalizeEmployeeLoanApplication({ ...request, witnesses: [...request.witnesses, { name: "Witness 4", address: "Address 4", phone: "01700000004" }, { name: "Witness 5", address: "Address 5", phone: "01700000005" }] }).witnesses.length, 5);
  assert.throws(() => normalizeEmployeeLoanApplication({ ...request, witnesses: request.witnesses.slice(0, 1) }), /at least two/i);
  assert.throws(() => normalizeEmployeeLoanApplication({ ...request, witnesses: [...request.witnesses, { name: "Witness 4", address: "Address 4", phone: "01700000004" }, { name: "Witness 5", address: "Address 5", phone: "01700000005" }, { name: "Witness 6", address: "Address 6", phone: "01700000006" }] }), /no more than five/i);
...
test("document upload redirects preserve successful objects while genuine failures clean up", async () => {
  const employeeActions = await read("app/employee/loans/actions.ts");
  const adminActions = await read("app/admin/receivables/employee-loan-actions.ts");
  for (const source of [employeeActions, adminActions]) {
    assert.match(source, /isRedirectError\(error\)\) throw error/);
  }
  assert.match(employeeActions, /if \(isRedirectError\(error\)\) throw error;[\s\S]*if \(storagePath\) await db\.storage\.from\(EMPLOYEE_LOAN_DOCUMENT_BUCKET\)\.remove/);
  assert.match(adminActions, /if \(isRedirectError\(error\)\) throw error;[\s\S]*if \(proofPath\) await db\.storage\.from\(EMPLOYEE_LOAN_DOCUMENT_BUCKET\)\.remove/);
});
...
test("employee pages and document routes enforce own-account access", async () => {
  ...
  assert.match(employeeDocument, /getCurrentProfile/);
  assert.match(employeeDocument, /createSignedUrl/);
  assert.match(employeeDocument, /metadataError/);
  assert.match(employeeDocument, /status: 503/);
  ...
  assert.match(adminDocument, /metadataError/);
  assert.match(adminDocument, /status: 503/);
});

test("migration keeps applications non-financial and gates activation on signed agreement", async () => {
  const migration = await read("supabase/migrations/202609110001_employee_loan_application_agreement.sql");
  ...
  assert.match(migration, /jsonb_array_length\(requested_witnesses\)\s*<\s*2/);
  assert.match(migration, /jsonb_array_length\(requested_witnesses\)\s*>\s*5/);
  assert.doesNotMatch(migration, /insert into public\.(journal|cashbook|hr_payroll)/i);
});

test("Admin Employee Loan workflow is integrated into production Receivables UI", async () => {
  const list = await read("app/admin/receivables/loans/page.tsx");
  const detail = await read("app/admin/receivables/loans/[id]/page.tsx");
  assert.match(list, /ReceivablesNavigation/);
  assert.match(list, /getNonSalesReceivables/);
  assert.match(detail, /ReceivablesNavigation/);
  assert.match(detail, /getAdminEmployeeLoanDetail/);
  assert.match(detail, /EmployeeLoanAdminPanel/);
  assert.match(await read("app/admin/receivables/employee-loan-actions.ts"), /record_receivable_repayment/);
});
