...
test("current-main Admin loan surfaces stay isolated from historical Receivables UI", async () => {
  ...
  for (const source of [list, detail]) {
    assert.doesNotMatch(source, /ReceivableOperations|ReceivablesNavigation|ReceivableAccountForms|lib\/receivables\/data|reporting-access/);
    assert.match(source, /DashboardShell/);
    assert.match(source, /requireAllPermissions/);
  }
  assert.match(list, /getAdminEmployeeLoanApplications/);
  assert.match(detail, /getAdminEmployeeLoanDetail/);
  assert.match(detail, /recordEmployeeLoanRepaymentAction/);
