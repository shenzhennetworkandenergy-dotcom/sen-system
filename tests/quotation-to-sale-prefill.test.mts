...
const optionMergePath = "lib/orders/creation-options.ts";
const optionMergeModule = existsSync(optionMergePath)
  ? await import("../lib/orders/creation-options.ts")
  : ({} as Record<string, unknown>);
...
function optionMergeFunction<T extends (...args: never[]) => unknown>(name: string) {
  const candidate = (optionMergeModule as Record<string, unknown>)[name];
  assert.equal(typeof candidate, "function", `${name} must be exported`);
  return candidate as T;
}
...
test("quotation submission retains every sourced row even outside bounded product options", () => {
  const submission = productionFunction<(
    rows: Array<Record<string, unknown>>,
    warehouse: string,
    validProducts: Set<string>,
    initial: QuotationSaleInitial,
    header: { discountAmount: string; serviceAmount: string; discountReason: string },
  ) => { items: Array<Record<string, unknown>> }>("buildSaleBuilderSubmission");
  const outsideProductId = "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa";
  const outsideItemId = "bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb";
  const reason = "Approved quotation QT-20260823-ZERO";
  const result = submission([
    {
      key: "first",
    ...
    {
      key: "outside",
      product_id: outsideProductId,
      variation_id: "",
      source_quotation_item_id: outsideItemId,
      quantity: "1",
      unit_price: "12.5",
      line_discount: "1",
      discount_percent: "0",
      line_tax: "0",
      reason,
      catalogue_price: 12.5,
      baseline_unit_price: 12.5,
      baseline_line_discount: 1,
    },
  ], warehouseId, new Set([productId]), quotation, {
    discountAmount: "4.25",
    serviceAmount: "0",
    discountReason: reason,
  });
  ...
  assert.deepEqual(
    result.items.map((item) => item.source_quotation_item_id),
    [quotationItemId, outsideItemId],
  );
  assert.deepEqual(
    result.items.map((item) => item.product_id),
    [productId, outsideProductId],
  );
});

test("manual submission preserves the raw adjustment reason exactly", () => {
  const submission = productionFunction<(
    rows: Array<Record<string, unknown>>,
    warehouse: string,
    validProducts: Set<string>,
  ...
  const rawReason = "  Manual reason with spaces  ";
  const result = submission([{
    key: "manual",
    product_id: productId,
    variation_id: "",
    ...
    baseline_unit_price: 10,
    baseline_line_discount: 0,
  }], warehouseId, new Set([productId]), undefined, {
    discountAmount: "0",
    serviceAmount: "0",
    discountReason: "",
  });

  assert.equal(result.items[0]?.adjustment_reason, rawReason);
});

test("sourced rows lock identity and removal while added rows remain selectable", () => {
  const controls = productionFunction<(
    row: { source_quotation_item_id: string | null },
  ) => { productLocked: boolean; variationLocked: boolean; removable: boolean }>(
    "saleBuilderRowControlState",
  );
  assert.deepEqual(controls({ source_quotation_item_id: quotationItemId }), {
    productLocked: true,
    variationLocked: true,
    removable: false,
  });
  assert.deepEqual(controls({ source_quotation_item_id: null }), {
    productLocked: false,
...
test("required creation options merge exact customer, addresses, products, variations, and zero balances", () => {
  const merge = optionMergeFunction<(
    bounded: Record<string, Array<Record<string, unknown>>>,
    exact: Record<string, Array<Record<string, unknown>>>,
    requirements: Record<string, unknown>,
  ) => Record<string, Array<Record<string, unknown>>> | null>(
    "mergeRequiredOrderCreationOptions",
  );
  const outsideProductId = "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa";
  const outsideVariationId = "bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb";
  const bounded = {
    customers: [{ id: "cccccccc-cccc-4ccc-8ccc-cccccccccccc", full_name: "Bounded" }],
    products: [{ id: productId, name: "Stale bounded product" }],
    variations: [],
    balances: [{ product_id: productId, variation_id: null, warehouse_id: warehouseId, available: 3 }],
    warehouses: [{ id: warehouseId, name: "Main" }],
    addresses: [],
  };
  const exact = {
    customers: [{ id: customerId, full_name: "Exact customer" }],
    products: [
      { id: productId, name: "Exact existing product" },
      { id: outsideProductId, name: "Exact outside product" },
    ],
    variations: [{ id: outsideVariationId, product_id: outsideProductId, name: "Blue / XL" }],
    balances: [{ product_id: outsideProductId, variation_id: outsideVariationId, warehouse_id: warehouseId, available: 0 }],
    addresses: [
      { id: shippingAddressId, profile_id: customerId },
      { id: billingAddressId, profile_id: customerId },
    ],
  };
  const requirements = {
    customerId,
    addressIds: [shippingAddressId, billingAddressId],
    lines: [
      { productId, variationId: null },
      { productId: outsideProductId, variationId: outsideVariationId },
    ],
  };
  const result = merge(bounded, exact, requirements);

  assert.ok(result);
  assert.deepEqual(result.customers.map((item) => item.id), [
    bounded.customers[0]?.id,
    customerId,
  ]);
  assert.deepEqual(result.products.map((item) => [item.id, item.name]), [
    [productId, "Exact existing product"],
    [outsideProductId, "Exact outside product"],
  ]);
  assert.deepEqual(result.variations.map((item) => [item.id, item.name]), [
    [outsideVariationId, "Blue / XL"],
  ]);
  assert.deepEqual(result.addresses.map((item) => item.id), [shippingAddressId, billingAddressId]);
  assert.deepEqual(result.balances.at(-1), exact.balances[0]);
  assert.equal(result.balances.at(-1)?.available, 0);
  assert.equal(new Set(result.products.map((item) => item.id)).size, result.products.length);
});

test("required creation options fail closed for missing or foreign exact records and preserve manual input by identity", () => {
  const merge = optionMergeFunction<(
    bounded: Record<string, Array<Record<string, unknown>>>,
    exact: Record<string, Array<Record<string, unknown>>>,
    requirements?: Record<string, unknown>,
  ...
  const bounded = { customers: [], products: [], variations: [], balances: [], warehouses: [], addresses: [] };
  assert.equal(merge(bounded, bounded), bounded);
  const exact = {
    customers: [{ id: customerId }],
    products: [{ id: productId }],
    variations: [{ id: variationId, product_id: productId }],
    balances: [],
    addresses: [{ id: shippingAddressId, profile_id: customerId }],
  };
  const requirements = {
    customerId,
    addressIds: [shippingAddressId],
    lines: [{ productId, variationId }],
  };
  assert.equal(merge(bounded, { ...exact, customers: [] }, requirements), null);
  assert.equal(merge(bounded, {
    ...exact,
    addresses: [{ id: shippingAddressId, profile_id: "cccccccc-cccc-4ccc-8ccc-cccccccccccc" }],
  }, requirements), null);
  assert.equal(merge(bounded, { ...exact, products: [] }, requirements), null);
  assert.equal(merge(bounded, {
    ...exact,
    variations: [{ id: variationId, product_id: "cccccccc-cccc-4ccc-8ccc-cccccccccccc" }],
  }, requirements), null);
});
...
test("SaleBuilder wiring selects the conversion action only for quotation mode and exposes no import mutation", () => {
  ...
  assert.match(builder, /locked=\{rowControls\.productLocked\}/);
  assert.match(builder, /disabled=\{rowControls\.variationLocked\}/);
  assert.match(builder, /disabled=\{!rowControls\.removable/);
...
test("locked customer and page/entry contracts preserve manual mode and authorize quotation mode", () => {
  ...
  const orderData = readFileSync("lib/orders/data.ts", "utf8");
  ...
  assert.match(newPage, /getOrderCreationOptions\(initialQuotation\s*\?/);
  assert.match(orderData, /required\.customerId/);
  assert.match(orderData, /required\.addressIds/);
  assert.match(orderData, /required\.lines/);
  assert.match(orderData, /\.in\("id", productIds\)/);
  assert.match(orderData, /\.in\("id", variationIds\)/);
  assert.match(orderData, /\.in\("product_id", productIds\)/);
