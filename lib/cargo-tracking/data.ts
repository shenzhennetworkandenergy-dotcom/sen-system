...
export const cargoTrackingEditableStatuses = cargoStatuses.slice(
  0,
  cargoStatuses.indexOf("dispatched_china"),
) as CargoStatus[];

export const canEditCargoTrackingNumber = (status: string) =>
  cargoTrackingEditableStatuses.includes(status as CargoStatus);
...
export async function getCargoTrackingEditJob(jobId: string) {
  const { data, error } = await createSupabaseAdminClient().from("cargo_shipping_jobs")
    .select("id,internal_cargo_id,courier_tracking_number,current_status")
    .eq("id", jobId).maybeSingle();
  if (error) throw new Error("Unable to load cargo tracking job.");
  return data;
}
