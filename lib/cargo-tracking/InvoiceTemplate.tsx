...
export function CargoInvoiceTemplate({ job, invoice, packages = [], customer, showPrint = true }: InvoiceTemplateProps) {
  ...
  return <main className="cargo-invoice mx-auto max-w-5xl bg-white text-slate-900 shadow-xl print:shadow-none">
    ...
    <style>{`@media print {
      ...
      .sen-dashboard-shell:has(.cargo-invoice) { min-height: 0 !important; }
      .sen-dashboard-shell:has(.cargo-invoice) .sen-dashboard-header,
      .sen-dashboard-shell:has(.cargo-invoice) .sen-dashboard-sidebar,
      .sen-dashboard-shell:has(.cargo-invoice) .sen-dashboard-title,
      .sen-dashboard-shell:has(.cargo-invoice) button[aria-controls="dashboard-navigation"],
      .sen-dashboard-shell:has(.cargo-invoice) button[aria-label="Close dashboard navigation"] { display: none !important; }
      .sen-dashboard-shell:has(.cargo-invoice) > div { display: block !important; max-width: none !important; margin: 0 !important; padding: 0 !important; }
      .sen-dashboard-shell:has(.cargo-invoice) .sen-dashboard-content { min-width: 0 !important; }
      .cargo-invoice { width: 210mm !important; max-width: 210mm !important; margin: 0 auto !important; box-shadow: none !important; print-color-adjust: exact; -webkit-print-color-adjust: exact; }
      .cargo-invoice * { print-color-adjust: exact; -webkit-print-color-adjust: exact; }
      .cargo-invoice > header, .cargo-invoice section, .cargo-invoice table, .cargo-invoice footer { break-inside: avoid; page-break-inside: avoid; }
    }`}</style>
  </main>;
}
