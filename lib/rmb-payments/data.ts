...
export type RmbAdminCustomerOption = CustomerSearchOption & {
...
export type RmbPaymentJob = {
  ...
  adjusted_rmb_amount: number | null;
...
export type RmbCustomerVisibleJob = Pick<
  RmbPaymentJob,
  | "id"
  | "job_reference"
  | "foreign_currency"
  | "foreign_amount"
  | "agreed_bdt_rate"
  | "calculated_bdt_payable"
  | "customer_payment_date"
  | "customer_payment_method"
  | "customer_payment_reference"
  | "customer_payment_note"
  | "customer_payment_proof_path"
  | "payee_organization"
  | "payee_name"
  | "payee_address"
  | "payee_phone"
  | "payee_account_details"
  | "china_payment_proof_path"
  | "china_destination_qr_path"
...
export async function getRmbCustomerOptions(): Promise<RmbAdminCustomerOption[]> {
  ...
  const customers = (data ?? []) as CustomerSearchOption[];
  if (!customers.length) return [];
  const { data: preferences, error: preferenceError } = await db
    .from("rmb_customer_preferences")
    .select("customer_id,service_percentage")
    .in("customer_id", customers.map((customer) => customer.id));
  if (preferenceError) throw new Error("Unable to load RMB customer preferences.");
  const percentageByCustomer = new Map(
    (preferences ?? []).map((item: { customer_id: string; service_percentage: number }) => [
      item.customer_id,
      Number(item.service_percentage),
    ]),
  );
  return customers.map((customer) => ({
    ...customer,
    service_percentage: percentageByCustomer.get(customer.id) ?? null,
  }));
}
...
export async function getRmbCustomerJob(jobId: string, customerId: string): Promise<RmbCustomerJobDetail | null> {
  ...
  const { data: rawJob, error: jobError } = await db
    .from("rmb_payment_jobs")
    .select("id,job_reference,foreign_currency,foreign_amount,agreed_bdt_rate,calculated_bdt_payable,customer_payment_date,customer_payment_method,customer_payment_reference,customer_payment_note,customer_payment_proof_path,payee_organization,payee_name,payee_address,payee_phone,payee_account_details,china_payment_proof_path,china_destination_qr_path,customer_instruction,current_status,created_at,updated_at")
    .eq("id", jobId)
    .eq("customer_id", customerId)
    .maybeSingle();
  if (jobError) throw new Error("Unable to load your RMB request.");
  if (!rawJob) return null;
  const job = rawJob as RmbCustomerVisibleJob;
