...
import {
  ...
  productListPageHref as buildProductListPageHref,
  type ProductListParams,
} from "@/lib/inventory/product-list-view";

export type { ProductListParams };

export function productListPageHref(params: ProductListParams, page: number) {
  return buildProductListPageHref(params, page);
}
...
export async function getProductList(params: ProductListParams) {
  ...
  const featuredFilter = normalizeFeaturedFilter(params.featured);
  ...
  const related = ids.length ? await Promise.all([
    ...
    db.from("product_categories").select("id,name"), db.from("product_media").select("product_id,storage_path,alt_text,is_primary,sort_order").in("product_id", ids).is("variation_id", null).eq("media_type", "image").order("sort_order"),
  ...
  const preferredMediaByProduct = new Map<string, (typeof media)[number]>();
  for (const id of ids) {
    const preferred = pickProductListImage(media, id);
    if (preferred) preferredMediaByProduct.set(id, preferred);
  }
  const mediaPaths = [...preferredMediaByProduct.values()].map((item) => item.storage_path);
  ...
  const products = (data ?? []).map((product) => {
    ...
    const preferredMedia = preferredMediaByProduct.get(product.id) ?? null;
    const image = preferredMedia ? { ...preferredMedia, signedUrl: mediaUrlMap.get(preferredMedia.storage_path) ?? null } : null;
