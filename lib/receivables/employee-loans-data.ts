...
export async function getEmployeeLoanIdentity() {
  ...
  const { data, error } = await createSupabaseAdminClient()
    ...
    .select("id,profile_id,employee_number,job_title,employment_status,profiles:profiles!hr_employee_records_profile_id_fkey(full_name,email,phone,address_line,city,region,postal_code),hr_departments(name),hr_designations(name),hr_employee_profiles(national_id,father_name,mother_name,present_address,permanent_address)")
...
export async function getAdminEmployeeLoanExtension(accountId: string) {
  ...
  const [employee, documents, representatives, disbursement] = await Promise.all([
    db.from("hr_employee_records")
      .select("id,employee_number,job_title,profiles:profiles!hr_employee_records_profile_id_fkey(full_name,email,phone,address_line,city,region,postal_code),hr_departments(name),hr_designations(name),hr_employee_profiles(national_id,father_name,mother_name,present_address,permanent_address)")
...
export async function getAdminEmployeeLoanDetail(accountId: string) {
  ...
  const [{ data: account, error: accountError }, { data: installments, error: installmentsError }, { data: transactions, error: transactionsError }, { data: audit, error: auditError }] = await Promise.all([
    db.from("non_sales_receivable_details_v")
      .select("id,receivable_number,employee_record_id,category,requested_amount,approved_amount,currency,status,installment_count,installment_amount,first_due_date,final_due_date,disbursement_date,created_at,approved_at,outstanding_amount,recovered_amount,disbursed_amount,opening_principal,opening_as_of_date,default_repayment_method,notes")
      .eq("id", accountId).eq("category", "employee_loan").maybeSingle(),
    db.from("receivable_installment_status_v")
      .select("id,installment_number,due_date,amount_due,paid_amount,remaining_amount,status:installment_status")
