...
export async function getHrEmployee(id: string) {
  ...
  const results = await Promise.all([
    ...
    db.from("hr_employee_profiles").select("id,employee_record_id,preferred_name,date_of_birth,gender,nationality,national_id,passport_number,personal_email,personal_phone,present_address,permanent_address,blood_group,marital_status,bank_name,bank_account_name,bank_account_number,bank_routing_number,tax_identifier,notes,updated_by,created_at,updated_at,father_name,mother_name").eq("employee_record_id",id).maybeSingle(),
