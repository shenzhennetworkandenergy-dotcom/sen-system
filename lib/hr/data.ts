...
export async function getHrDashboard() {
  ...
  const [employees, departments, profiles, locations, leave] = await Promise.all([
    db.from("hr_employee_records").select("id,profile_id,employee_number,job_title,employment_type,employment_status,hire_date,base_salary,salary_currency,hr_departments(name),profiles:profiles!hr_employee_records_profile_id_fkey(full_name,email),work_locations(name)").order("created_at", { ascending: false }),
    ...
    db.from("hr_leave_requests").select("id,leave_type,start_date,end_date,status,reason,hr_employee_records(employee_number,profiles:profiles!hr_employee_records_profile_id_fkey(full_name,email))").order("created_at", { ascending: false }).limit(50),
  ]);
